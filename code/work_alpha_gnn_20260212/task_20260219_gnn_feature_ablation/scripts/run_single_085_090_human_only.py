...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Root of the deliverables/ directory (auto-detected from script location).",
    )
    parser.add_argument(
        "--prefiltered_data_root",
        default=None,
        help="Path to pre-filtered human-only data dir (e.g. data/human_only_085_090). "
             "When given, skips organism filtering and master_meta_csv is not required.",
    )
    parser.add_argument(
        "--master_meta_csv",
        default=None,
        help="Master metadata CSV with GS_ID/ORGANISM columns. "
             "Only needed when --prefiltered_data_root is not given.",
    )
    ...
    base = root / "code" / "work_alpha_gnn_20260212"
    task = base / "task_20260219_gnn_feature_ablation"
    scripts_base = base / "scripts"
    scripts_task = task / "scripts"
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")

    run_root = root / "outputs" / "runs" / f"single_085_090_human_only_{ts}"
    plot_dir = root / "outputs" / "plots" / f"single_085_090_human_only_{ts}"
    ...
    if args.prefiltered_data_root:
        pdr = Path(args.prefiltered_data_root).resolve()
        subset_cfg = {
            "085": SubsetConfig(
                subset="085",
                edge_file=pdr / "subset085" / "edges_with_mtype_threshold_085_human.txt",
                meta_csv=pdr / "subset085" / "enriched_gs_metadata_subset085_human.csv",
                go_npz=pdr / "subset085" / "go_embeddings_subset_085_human_l2.npz",
                pager_npz=pdr / "subset085" / "pager_embeddings_subset_085_human_l2.npz",
                fusion_npz=pdr / "subset085" / "fusion_subset_085_human.npz",
                gnn_npz=pdr / "subset085" / "gnn_old_holdout_subset_085_human.npz",
            ),
            "090": SubsetConfig(
                subset="090",
                edge_file=pdr / "subset090" / "edges_with_mtype_threshold_090_human.txt",
                meta_csv=pdr / "subset090" / "enriched_gs_metadata_subset090_human.csv",
                go_npz=pdr / "subset090" / "go_embeddings_subset_090_human_l2.npz",
                pager_npz=pdr / "subset090" / "pager_embeddings_subset_090_human_l2.npz",
                fusion_npz=pdr / "subset090" / "fusion_subset_090_human.npz",
                gnn_npz=pdr / "subset090" / "gnn_old_holdout_subset_090_human.npz",
            ),
        }
        master_org = None  # not needed when prefiltered data is provided
    else:
        if args.master_meta_csv is None:
            raise ValueError("--master_meta_csv is required when --prefiltered_data_root is not given.")
        subset_cfg = {
            "085": SubsetConfig(
                subset="085",
    ...
    for subset in ["085", "090"]:
        ...
        if args.prefiltered_data_root:
            # Use pre-filtered files directly — no organism filtering needed.
            meta_h_path = cfg.meta_csv
            edge_h_path = cfg.edge_file
            go_h = cfg.go_npz
            pager_h = cfg.pager_npz
            fusion_h = cfg.fusion_npz
            gnn_h = cfg.gnn_npz
            # KG scripts (build_kg.py) expect these three files at inputs/ inside
            # the run dir. Symlink them from the prefiltered data root.
            for _src, _name in [
                (cfg.gnn_npz,   f"gnn_old_holdout_subset_{subset}_human.npz"),
                (cfg.meta_csv,  f"enriched_gs_metadata_subset{subset}_human.csv"),
                (cfg.edge_file, f"edges_with_mtype_threshold_{subset}_human.txt"),
            ]:
                _dst = inputs_dir / _name
                if not _dst.exists():
