...
# Paths
# ---------------------------------------------------------------------------
PROJECT_ROOT = Path(__file__).resolve().parents[2]  # deliverables/


def _find_latest_run_dir(root: Path) -> Path:
    """Find the most recent single_085_090_human_only run directory.

    Searches outputs/runs/ first (new runs produced by run_pipeline.sh),
    then rerun/ (pre-existing reference runs shipped with the deliverable).
    """
    candidates = []
    for search_dir in [root / "outputs" / "runs", root / "rerun"]:
        if search_dir.exists():
            for d in search_dir.iterdir():
                if d.is_dir() and d.name.startswith("single_085_090_human_only_"):
                    candidates.append(d)
    if not candidates:
        raise FileNotFoundError(
            f"No single_085_090_human_only_* run directory found under {root}. "
            "Run run_pipeline.sh first, or check outputs/ and rerun/ directories."
        )
    return sorted(candidates, key=lambda p: p.name)[-1]  # latest by dir name (timestamp-sorted)


RUN_DIR = _find_latest_run_dir(PROJECT_ROOT)
SUBSET_DIR = RUN_DIR / "subset085"
